...
** Last update	Thu Feb 25 11:53:30 2016 Adrien WERY
*/
    ...
    while (i < max)
    {
        ...
        if (sym->st_shndx != SHN_ABS && name[0] != '\0')
        {
            ...
            if ((syms[nb_syms].type = getSymType(sym)) == '?')
                syms[nb_syms].type = GLOBAL(getSectionType(&G_SHDR[sym->st_shndx],
                        &elf->shstrtab[G_SHDR[sym->st_shndx].sh_name]));
            ++nb_syms;
        }
        ++i;
    }
...
void    display64(t_elf *elf)
{
    ...
    elf->shstrtab = elf->data + G_SHDR[Ehdr->e_shstrndx].sh_offset;
    i = 0;
    while (i < Ehdr->e_shnum)
    {
        if (G_SHDR[i].sh_type == SHT_SYMTAB)
        {
            displaySym64(elf, G_SHDR[i].sh_size / G_SHDR[i].sh_entsize,
                (size_t)elf->data + G_SHDR[i].sh_offset,
                elf->data + G_SHDR[G_SHDR[i].sh_link].sh_offset);
